...
def _candidate_repo(repo: Path, receipt: dict[str, object]) -> Path:
    ...
    if stage == TaskStage.IMPLEMENTING_UNBORN:
        return repo
...
def _begin(args: argparse.Namespace) -> list[str]:
    ...
    lifecycle = receipt.get("lifecycle") if isinstance(receipt.get("lifecycle"), dict) else {}
    stage = TaskStage.parse(lifecycle.get("stage"), default=TaskStage.BRIEFED)
    if session.get("phase") == "audit" and stage in {
        ...
        lines.append(f"GOVERNANCE-PHASE: {session.get('phase')}")
    return lines
...
def _status(args: argparse.Namespace) -> list[str]:
    ...
    lifecycle = receipt.get("lifecycle") if isinstance(receipt.get("lifecycle"), dict) else {}
    stage = TaskStage.parse(lifecycle.get("stage"), default=TaskStage.BRIEFED)
    completed = stage == TaskStage.COMPLETED
    lines = [
...
def _defer(args: argparse.Namespace) -> CommandResult:
    repo = git.root()
    receipt, session = _session(repo, args.task_id)
    if session.get("phase") == "deferred":
        result = tasks.prepare_finish(
            repo,
            args.task_id,
            subject=f"Record deferred governance audit {session.get('audit') or ''}",
        )
        return CommandResult(result.lines, result.data, result.outcome)
    if session.get("phase") not in {"decision", "adopt"}:
    ...
    return CommandResult(
        ["GOVERNANCE-PHASE: deferred", *result.lines],
        result.data,
        result.outcome,
    )
