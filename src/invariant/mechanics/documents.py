...
def create_yaml(path: Path, value: Any) -> bool:
    """Atomically create `path` with `value`; return False without writing when it already exists."""
    ...
    try:
        with os.fdopen(descriptor, "w", encoding="utf-8") as handle:
            yaml.dump(value, handle, Dumper=LiteralDumper, sort_keys=False, allow_unicode=True, width=100)
        try:
