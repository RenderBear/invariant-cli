...
def _valid_boundary(value: str) -> bool:
    try:
        BoundaryDisposition.parse(value)
...
def _automatic_open_authority(
    ...
    if not reach.needs_explicit_authority or boundary == "unresolved":
        return False
    resolved = _target_config(repo, target)
    accepted = (
        resolved
        if base == "unborn"
        else config.resolve_at(repo, base, target)
    )
...
def _transition(
    receipt: dict[str, object], stage: TaskStage, branch: str, worktree: str
) -> dict[str, object]:
    receipt["lifecycle"] = {
        "stage": stage.value,
        "branch": branch,
...
def _activate(
    ...
    *,
    ...
    if head == "unborn":
        branch = target
        stage = TaskStage.IMPLEMENTING_UNBORN
        worktree = repo
    else:
        branch = _branch_name(repo, task, head)
        worktree = _worktree_path(repo, branch)
        if execution == "assisted":
            stage = TaskStage.AWAITING_BRANCH
        else:
            try:
    ...
    output.append(
        ...
        if stage == TaskStage.AWAITING_BRANCH
...
def begin(
    ...
    *,
    ...
    if _blocking_hook_requests(receipt) and lifecycle.get("stage") in {
        ...
        TaskStage.IMPLEMENTING_UNBORN.value,
    }:
        receipt = _transition(
            receipt,
            TaskStage.BRIEFING,
...
def _status_lines(repo: Path, receipt: dict[str, object]) -> list[str]:
    lifecycle = receipt.get("lifecycle") if isinstance(receipt.get("lifecycle"), dict) else {}
    stage = TaskStage.parse(lifecycle.get("stage"), default=TaskStage.BRIEFED)
...
def status(repo: Path, task: str) -> list[str]:
    ...
    if not path.is_file():
        ...
        raise Blocked(
            f"Invariant: no active or completed task '{task}'",
            code="missing_task",
            lines=[f"TASK: {task}", "STATUS: absent"],
        )
    receipt = receipts.load(repo, task)
    return [*_status_lines(repo, receipt), *_hook_lines(receipt)]
...
def respond(repo: Path, task: str, request_id: str, source: str) -> FlowResult:
    ...
    lifecycle = receipt.get("lifecycle") if isinstance(receipt.get("lifecycle"), dict) else {}
    stage = TaskStage.parse(lifecycle.get("stage"), default=TaskStage.BRIEFED)
    if not _blocking_hook_requests(receipt) and stage == TaskStage.BRIEFING:
        receipt.pop("goal", None)
        active_stage = (
            TaskStage.IMPLEMENTING_UNBORN
            if str(receipt.get("integration_head")) == "unborn"
            else TaskStage.IMPLEMENTING
        )
        receipt = _transition(
            receipt,
            active_stage,
        ...
        return FlowResult(
            ...
            {"task": task, "stage": active_stage.value, "actions": []},
            CommandOutcome.READY,
        )
    if not _blocking_hook_requests(receipt) and stage == TaskStage.AWAITING_REVIEW:
        active_stage = (
            TaskStage.IMPLEMENTING_UNBORN
            if str(receipt.get("integration_head")) == "unborn"
            else TaskStage.IMPLEMENTING
    ...
    return FlowResult(
        ...
        CommandOutcome.NEEDS_INPUT if actions else CommandOutcome.READY,
    )
...
def _actual_paths(
    repo: Path, stage: TaskStage, base: str, branch: str
) -> tuple[str | None, list[str]]:
    if stage == TaskStage.IMPLEMENTING:
...
def _candidate_repo(repo: Path, active_stage: TaskStage, branch: str) -> Path:
    if active_stage == TaskStage.IMPLEMENTING_UNBORN:
        return repo
    worktree = git.worktree_for_branch(repo, branch)
    if worktree is None:
        raise Blocked(
...
def _complete_task(
    repo: Path, task: str, active_stage: TaskStage, branch: str, target: str
) -> None:
    if active_stage == TaskStage.IMPLEMENTING:
        primary = git.primary_worktree(repo).resolve()
        worktree = git.worktree_for_branch(repo, branch)
        if worktree is not None and worktree.resolve() != primary:
            ...
            if removed.returncode:
                receipts.set_lifecycle(
                    repo, task, TaskStage.CLEANUP_REQUIRED, branch, str(worktree)
                )
...
def finish(
    ...
    *,
    ...
    domains_at_base = (
        set(governance.domain_index(repo, base).identifiers)
        if base != "unborn"
        else set()
    )
    ...
    context = governance.context_result(
        candidate_repo,
        paths=actual_paths,
        base=None if active_stage == TaskStage.IMPLEMENTING_UNBORN else base,
        root_mode=active_stage == TaskStage.IMPLEMENTING_UNBORN,
    ...
    if expected_tree:
        evidenced_head = str(receipt.get("review_integration_head") or "")
        current_head = git.resolve(repo, f"refs/heads/{target}") or "unborn"
        if evidenced_head and evidenced_head != current_head:
            raise Blocked(
                f"Invariant: integration branch '{target}' advanced after candidate evidence was captured; integration branch unchanged",
                code="concurrent_ref_movement",
                lines=[
                    f"EVIDENCED-HEAD: {evidenced_head}",
                    f"ACTUAL-HEAD: {current_head}",
                    f"NEXT: rerun 'invariant task finish {task}' to re-verify the candidate against the advanced head",
                ],
            )
    if adapters.enabled(receipt):
        candidate_tree = landing.prospective_tree(
            repo,
            target,
            None if active_stage == TaskStage.IMPLEMENTING_UNBORN else branch,
        )
        adapters.run_hook(
            receipts.task_root(repo, task),
            receipt,
        ...
        if stage == TaskStage.AWAITING_REVIEW:
            _transition(
...
def _require_candidate_review(
    repo: Path,
    task: str,
    context: governance.ContextResult,
    disposition: str,
    target: str,
    base: str,
    ...
    raise Blocked(
        ...
        lines=[
            f"CANDIDATE-TREE: {candidate_tree}",
            f"REVIEWED-TREE: {review.candidate_tree if review else 'none'}",
            *[f"AFFECTED: {item}" for item in context.reviews],
            f"NEXT: run 'invariant task finish {task}' and resolve its review action with task respond",
        ],
    )
...
def prepare_assessment(repo: Path, task: str) -> tuple[dict[str, object], dict[str, object]]:
    ...
    previous_ref = None if active_stage == TaskStage.IMPLEMENTING_UNBORN else base
    current_domains = governance.domains(candidate_repo)
    previous_domains = (
        {domain.identifier: domain for domain in governance.domains(candidate_repo, previous_ref)}
        if previous_ref
        else {}
    )
    changed_domains = (
    ...
    reach = candidate_context.reach
    reviews = list(candidate_context.reviews)
    ...
    will_run = list(candidate_context.verifier_locators)
    governance_refs = [
        *[f"semantic:{row.identifier}" for row in changed_semantics if row.status == "active"],
        *[domain.reference for domain in changed_domains],
    ...
    if boundary == "unresolved":
        if governance_refs or durable_registry_changed:
            boundary = "recorded"
        elif reach in {Reach.LOCAL, Reach.BOUNDED}:
            boundary = "no-record"
    allow_open = _automatic_open_authority(repo, target, base, reach, boundary)
    candidate_tree = landing.prospective_tree(
        repo,
        target,
        None if active_stage == TaskStage.IMPLEMENTING_UNBORN else branch,
    ...
    if reach.needs_explicit_authority and not allow_open:
        required.append(
            {
                "field": "allow_open",
...
def prepare_finish(
    ...
    *,
    ...
    """Collect evidence and converge routine concurrent work without manual retry waves."""

    try:
        return _prepare_finish_once(repo, task, subject=subject, checks=checks)
    except Blocked as movement:
        if movement.code not in {
            "concurrent_ref_movement",
            "stale_adapter_review",
        }:
            raise
        receipt = receipts.load(repo, task)
        lifecycle = (
        ...
        stage = TaskStage.parse(lifecycle.get("stage"), default=TaskStage.BRIEFED)
        if stage != TaskStage.IMPLEMENTING or adapters.enabled(receipt):
            raise
        target = str(receipt.get("integration_target") or "")
        initial_movement = movement

    last_movement = initial_movement
    with landing.convergence_queue(repo, target):
        for attempt in range(1, landing.ROUTINE_CONVERGENCE_ATTEMPTS + 1):
            try:
                result = _prepare_finish_once(
                    repo, task, subject=subject, checks=checks
                )
            except Blocked as movement:
                if movement.code not in {
                    "concurrent_ref_movement",
            ...
            return FlowResult(
                [
                    f"CONVERGENCE: rebuilt and reverified after concurrent candidate movement ({attempt} retry)",
                    *result.lines,
                ],
                {**result.data, "convergence_retries": attempt},
                result.outcome,
            )
    raise last_movement


def _prepare_finish_once(
    repo: Path,
    task: str,
    *,
    subject: str | None = None,
    ...
    try:
        candidate, _, evidence, candidate_context = landing.collect_evidence(
            repo, evidence_request
        )
    except Blocked as exc:
        current = receipts.load(repo, task)
        current_lifecycle = (
            current.get("lifecycle") if isinstance(current.get("lifecycle"), dict) else {}
        )
        exc.lines.extend(
            [
                f"TASK: {task}",
                f"STATUS: {TaskStage.parse(current_lifecycle.get('stage'), default=TaskStage.IMPLEMENTING)}",
                "RECOVERY: receipt and task branch retained; integration target unchanged",
                f"NEXT: inspect with 'invariant task status {task}', correct the candidate, "
                "then rerun task finish",
            ]
        )
        raise
    ...
    disposition = (
        str(boundary.get("disposition") or "unresolved")
        if isinstance(boundary, dict)
        else "unresolved"
    )
    assessment["allow_open"] = _automatic_open_authority(
        repo,
        target,
        base,
        candidate_context.reach,
        disposition,
    )
    required = [
        item
        for item in analysis.get("required", [])
        if isinstance(item, dict)
        and item.get("field") not in {"architecture_reviews", "allow_open"}
    ]
    if exact_reviews:
        required.append(
            {
                "field": "architecture_reviews",
                "reason": "the final candidate review must acknowledge these affected decisions",
                "values": exact_reviews,
            }
        )
    if candidate_context.reach.needs_explicit_authority and not assessment["allow_open"]:
        required.append(
            {
                "field": "allow_open",
                "reason": "human semantic authority must approve this open or gated transition",
    ...
    for item in evidence:
        ...
        dump_yaml(
            evidence_root / f"{identifier.replace(':', '-')}.yml", item.as_dict()
        )
...
def continue_task(repo: Path, task: str, *, apply: bool = False) -> list[str]:
    ...
    branch = str(lifecycle.get("branch") or "")
    target = str(receipt.get("integration_target") or "")
    if stage in {TaskStage.IMPLEMENTING, TaskStage.IMPLEMENTING_UNBORN}:
        return _status_lines(repo, receipt)
    if stage not in {TaskStage.AWAITING_BRANCH, TaskStage.AWAITING_LANDING}:
        raise Blocked(f"Invariant: task '{task}' cannot continue from stage '{stage}'")
    if not apply:
        action = (
...
def task_guidance(repo: Path, task: str, *, full: bool = False) -> list[str]:
    ...
    if (
        stage == TaskStage.IMPLEMENTING
        and branch
        and captured_head not in {"", "unborn"}
    ):
        branch_ref = git.resolve(repo, f"refs/heads/{branch}")
        if branch_ref:
    ...
    elif stage == TaskStage.IMPLEMENTING_UNBORN:
    ...
    output.extend(
        [
            "",
            *guidance.for_stage(
                stage.value, full=full
            ).splitlines(),
        ]
    )
    adapter_phase = (
        TASK_CREATED
        if stage in {TaskStage.BRIEFING, TaskStage.AWAITING_BRANCH}
...
def _remove_task_work(repo: Path, branch: str, target: str, *, discard: bool) -> list[str]:
    """Remove a task's generated worktree and branch; refuse to drop uncommitted work silently."""
    ...
    if worktree is not None and worktree.resolve() != primary:
        removed = git.run(
            ["worktree", "remove", *(["--force"] if discard else []), str(worktree)],
            cwd=primary,
            check=False,
        )
        if removed.returncode:
            raise Blocked(
                f"Invariant: task worktree '{worktree}' still holds uncommitted work",
                code="work_retained",
                lines=[
                    f"GIT: {removed.stderr or removed.stdout or 'Git refused worktree removal'}",
                    "NEXT: commit or move that work, or rerun with --discard to drop it",
                ],
            )
        output.append(f"REMOVED: worktree {worktree}")
    elif worktree is not None and git.current_branch(primary) == branch and target:
        git.run(["switch", "-q", target], cwd=primary)
    if git.branch_exists(repo, branch):
        git.run(["branch", "-D", branch], cwd=primary)
        output.append(f"REMOVED: branch {branch}")
    return output


def _landed_commit(repo: Path, task: str, target: str, base: str, branch: str) -> str | None:
    """Find the integration commit that landed this task, if any, after its captured base."""

    if not target or not git.resolve(repo, f"refs/heads/{target}"):
        return None
    branch_head = git.resolve(repo, f"refs/heads/{branch}") if branch else None
    after = base if base and base != "unborn" and git.resolve(repo, base) else None
    for commit in git.commits_mentioning(repo, f"refs/heads/{target}", "Invariant-Unit", after=after):
        if task not in git.trailers(repo, commit, "Invariant-Unit"):
            continue
        if branch_head and not git.is_ancestor(repo, branch_head, commit):
            continue
        return commit
    return None


def reconcile(repo: Path, task: str) -> list[str]:
    """Repair a task whose landing outran its bookkeeping, or report that nothing is wrong."""
    ...
    base = str(receipt.get("integration_head") or "")
    if target:
        landing.replay_pending_sync(repo, target)
    landed = _landed_commit(repo, task, target, base, branch)
    if landed is None:
        if stage == TaskStage.CLEANUP_REQUIRED:
            raise InvariantError(
                f"Invariant: task '{task}' awaits cleanup but no landing carries its unit trailer",
                code="unrecoverable_task",
            )
        if not branch:
            raise Blocked(
                f"Invariant: task '{task}' has no lifecycle state to resume",
                code="unrecoverable_task",
                lines=[f"NEXT: invariant task invalidate {task}, then begin the task again"],
            )
        return [*_status_lines(repo, receipt), "RECONCILE: nothing to repair — the task is active and has not landed"]
    output = _remove_task_work(repo, branch, target, discard=False)
    boundary = git.trailers(repo, landed, "Invariant-Boundary")
    receipt["resolved_boundary"] = boundary[0] if boundary else "unresolved"
    receipt["candidate_tree"] = git.resolve(repo, f"{landed}^{{tree}}", "") or ""
    receipts.save(repo, task, receipt)
    receipts.complete(repo, task, landed)
    return [
        f"TASK: {task}",
        "STATUS: completed",
        f"LANDING-COMMIT: {landed}",
        *output,
        "RECONCILED: archived the landed task and released its work",
    ]


def invalidate(repo: Path, task: str, *, discard: bool = False) -> list[str]:
    try:
        receipt = receipts.load(repo, task)
    except Blocked as exc:
    ...
    target = str(receipt.get("integration_target") or "")
    base = str(receipt.get("integration_head") or "")
    if branch and not discard and git.branch_exists(repo, branch):
        merged = bool(target) and git.is_ancestor(repo, f"refs/heads/{branch}", f"refs/heads/{target}")
        counted = (
            git.run(["rev-list", "--count", f"{base}..refs/heads/{branch}"], cwd=repo, check=False)
            if base and base != "unborn"
            else None
        )
        unlanded = int(counted.stdout or 0) if counted and counted.returncode == 0 else 0
        if unlanded and not merged:
            raise Blocked(
                f"Invariant: task branch '{branch}' holds {unlanded} unlanded commit(s)",
                code="work_retained",
                lines=[
                    "NEXT: finish the task to land them, or rerun with --discard to drop them",
                ],
            )
    output = _remove_task_work(repo, branch, target, discard=discard)
    return [*receipts.invalidate(repo, task), *output]
