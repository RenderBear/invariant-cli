...
from invariant.mechanics import coordinate
...
def _harness_path(repo: Path) -> Path:
    return coordinate.runtime_root(repo) / "harness"
...
def set_repo_harness(repo: Path, value: str) -> None:
    ...
    runtime = coordinate.ensure_runtime(repo)
...
def _mode_path(repo: Path) -> Path:
    return coordinate.runtime_root(repo) / "session-mode"
