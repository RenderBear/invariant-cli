...
out=$(cd "$fixture" && "$cli" task finish establish-meaning)
printf '%s\n' "$out" | grep -q '^ACTION: core:candidate-review — review_semantics$' ||
  die "recording semantic meaning did not require a candidate review"
packet="$fixture/.invariant/runtime/tasks/establish-meaning/review-packet.yml"
cat >"$assessment" <<EOF
version: 1
review_id: $(sed -n 's/^review_id: //p' "$packet")
candidate_tree: $(sed -n 's/^candidate_tree: //p' "$packet")
verdict: accepted
summary: The records index the canonical prose without restating its argument.
semantic_effect: recorded
authority: user:task:establish-meaning#review
candidate_defects: []
retained_discoveries: []
EOF
out=$(cd "$fixture" && "$cli" task respond establish-meaning core:candidate-review --input "$assessment")
printf '%s\n' "$out" | grep -q '^STATUS: completed$' || die "reviewed semantic records did not land"
receipt=$(grep -l '^locator: command:checks/ordinary-source.sh$' "$fixture"/.invariant/runtime/verifications/*.yml | head -n 1)
[ -n "$receipt" ] || die "semantic record verifier did not run against the candidate"
grep -q '^status: passed$' "$receipt" || die "semantic record verifier evidence was not recorded as passed"
