...
review="$fixture-review.yml"
cleanup() { rm -rf "$fixture" "$assessment" "$review"; }
...
mkdir -p "$fixture/src" "$fixture/.invariant"
printf 'one\n' >"$fixture/src/a.txt"
...
push_remote: off
EOF
git -C "$fixture" add src/a.txt .invariant/config.yml
git -C "$fixture" commit -qm seed

ok() { echo "ok - $1"; }
die() { echo "not ok - $1"; exit 1; }

# Accept the pending core candidate review for a task with the given semantic effect.
respond_review() {
  packet="$fixture/.invariant/runtime/tasks/$1/review-packet.yml"
  cat >"$review" <<EOF
version: 1
review_id: $(sed -n 's/^review_id: //p' "$packet")
candidate_tree: $(sed -n 's/^candidate_tree: //p' "$packet")
verdict: accepted
summary: The exact candidate preserves the affected decisions.
semantic_effect: $2
authority: user:task:$1#review
candidate_defects: []
retained_discoveries: []
EOF
  (cd "$fixture" && "$cli" task respond "$1" core:candidate-review --input "$review")
}
...
allow_open: true
EOF
before=$(git -C "$fixture" rev-parse main)
if out=$(cd "$fixture" && "$cli" task finish governance-baseline --assessment "$assessment" 2>&1); then
  die "a caller-supplied assessment landed open governance without a candidate review"
fi
printf '%s\n' "$out" | grep -q '^Invariant: the candidate requires an accepted semantic review before landing$' ||
  die "assessment-only landing of governance was not refused as an unreviewed candidate"
[ "$(git -C "$fixture" rev-parse main)" = "$before" ] || die "unreviewed assessment moved main"
ok "an assessment file cannot assert reviews or open authority for the candidate"

out=$(cd "$fixture" && "$cli" task finish governance-baseline)
printf '%s\n' "$out" | grep -q '^STATUS: awaiting-review$' ||
  die "governance adoption did not pause for its candidate review"
printf '%s\n' "$out" | grep -q '^ACTION: core:candidate-review — review_semantics$' ||
  die "governance adoption did not expose the core review action"
out=$(respond_review governance-baseline recorded)
printf '%s\n' "$out" | grep -q '^STATUS: completed$' || die "reviewed governance adoption did not land"
receipt=$(grep -l '^locator: command:checks/source-contract.sh$' "$fixture"/.invariant/runtime/verifications/*.yml | head -n 1)
[ -n "$receipt" ] || die "new contract verifier did not run during governance adoption"
grep -q '^status: passed$' "$receipt" || die "new contract verifier evidence was not recorded as passed"
...
json=$(cd "$fixture" && "$cli" --format json context rows source)
printf '%s\n' "$json" | grep -q '"domains":\[{"id":"source"' ||
  die "domain context did not expose typed domain records"
printf '%s\n' "$json" | grep -q '"contracts":\[{"version"' &&
  die "domain context leaked a document wrapper into contract rows"
printf '%s\n' "$json" | grep -q '"id":"source.contract.v1"' ||
  die "domain context omitted contracts linked by the domain"
ok "domain context JSON exposes responsibilities and linked records without parsing text rows"
...
printf '%s\n' "$json" | grep -q '"topology":\["area.src"\]' || die "JSON topology is not structured"
...
out=$(cd "$fixture" && "$cli" task finish assisted-flow)
printf '%s\n' "$out" | grep -q '^STATUS: awaiting-review$' || die "assisted finish skipped the candidate review"
if out=$(respond_review assisted-flow no-record 2>&1); then
...
echo "9 CLI lifecycle checks passed"
